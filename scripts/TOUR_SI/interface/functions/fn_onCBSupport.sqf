...
switch (toLower _supportType) do
{
	...
	case "airstrike":
	{
...

		{
			_t = switch (toLower (_x select 0)) do
			{
				case "vn_bomb_500_blu1b_fb_ammo": {"500lb Napalm Bombs"};
				case "bo_mk82":{"500lb HE Bombs"};
				case "bombcluster_03_ammo_f" : {"500lb HE Cluster Bombs"};
				case "bombcluster_01_ammo_f" : {"750lb HE Cluster Bombs"};
				case "bombcluster_02_ammo_f" : {"1100lb HE Cluster Bombs"};
				default {""};
			};
			if (_t != "") then
			{
					...
					
		{
			_x ctrlShow true;
					...
					_comboType,
					_sliderAmountHeight, _sliderRadius,
					_textType, _textAmountHeight, _textRadius, _textIntSpeed];
		...
		_textType ctrlSetText "TYPE OF ORDENANCE:";
		_textAmountHeight ctrlSetText "AMOUNT OF ORDENANCE:";
		_textRadius ctrlSetText "DIRECTION:";
		_textIntSpeed ctrlSetText "DISTANCE:";
		_buttonExecute ctrlSetText "EXECUTE FIRE MISSION";
		...
		_textInfo =
		"
			<img size='3' image='\a3\ui_f\data\map\vehicleicons\iconplane_ca.paa'/> <t size='1.25' font='puristaMedium'>SELECT SUPPORT REQUEST</t>
			<br/>
			<br/>
			<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>AIRSTRIKE: Use the options below and select a point on the map to customize your plane airtrike support, and press REQUEST FIRE MISSION.</t>
		";
	};	
	...
	case "helicopter":
	{
		...
		lbClear _comboBehaviour;
		{
			...
			if (!isNull (_item select 1)) then
			{
				if (!isNull (driver (_item select 1))) then
				{
					if !(grpNull == (_item select 0)) then
					{
						if (group (driver (_item select 1)) == (_item select 0)) then
						{
							if (canMove (_item select 1)) then
							{
								if (call _condition) then 
								{
									_grpName = (groupId (_item select 0));
			...
			if (!isNull (_item select 1)) then
			{
				if (!isNull (driver (_item select 1))) then
				{
					if !(grpNull == (_item select 0)) then
					{
						if (group (driver (_item select 1)) == (_item select 0)) then
						{
							if (((canMove (_item select 1)) or (!isNil {(_item select 1) getVariable "TOUR_SI_unitFuel"})) && (damage (_item select 1) < 0.7)) then
							{
								if (call _condition) then 
								{
									_grpName = (groupId (_item select 0));
...
if ((str (getMarkerPos "TOUR_SI_markerAS")) != "[0,0,0]") then
{
	deleteMarkerLocal "TOUR_SI_markerAS";
};