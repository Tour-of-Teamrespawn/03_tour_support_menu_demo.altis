...
if (_text3 < 0) then
{
	...
	if (_supportType == "Airstrike") then
	{
		...
		_text = 
		"
			<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>DIRECTION NOT SET: Please select the direction of attack for your strike</t>
			<br/>
		";
	};
	if (_supportType == "Helicopter") then
...
switch (toLower _supportType) do
{
	...
	case "airstrike": {
				_textHelp ctrlSetStructuredText parseText format [
				"
				<img size='3' image='\a3\ui_f\data\map\vehicleicons\iconplane_ca.paa'/> <t size='1.25' font='puristaMedium'>ARTILLERY SUPPORT</t>
				<br/>
				<br/>
				%1
				", _text];
...
if (_supportType == "Airstrike") exitWith
{
	[_position, _text1, _text2, _text3, _text4, player] remoteExec ["TOUR_SI_fnc_radio_EAM", 0, false];
};
