...
switch (toLower _supportType) do
{
	...
	case "airstrike":
	{
		...
		if(_commandType == "nothing") exitWith 
		{
			["error", "No airstrike ordenance of that type found!"] call TOUR_SI_fnc_hint;
		};
		if(_commandType == "") exitWith 
		{
			["info", "No airstrike ordenance remaining!"] call TOUR_SI_fnc_hint;
		};
		...
		if (_ammo < 1) exitWith
		{
			...
			_textHelp ctrlSetStructuredText parseText format [
			"
				<img size='3' image='\a3\ui_f\data\map\vehicleicons\iconplane_ca.paa'/> <t size='1.25' font='puristaMedium'>AIRSTRIKE SUPPORT</t>
				<br/>
				<br/>
				<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>ORDENANCE TYPE: %1</t>
				<br/>
				<br/>
				<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>There are no rounds left for this ordenance type. Please select another type of ammunition.</t>
			", _commandType];
		};

		_sliderAmountHeight sliderSetRange [1, _ammo];
		_SliderIntSpeed sliderSetRange [3, 60];
		_SliderRadius sliderSetRange [0, 360];
		...
		
		{
			_x ctrlShow true;
					...
					_textType, _textAmountHeight, _textRadius];

		{
			_x ctrlShow false;
					...
					_comboUnit, _comboBehaviour,
					_sliderIntSpeed,
					_textUnit, _textBehaviour,_textIntSpeed];
		...
		"
			...
			<br/>
			<br/>
			<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>DIRECTION: %3 Degrees</t>
			<br/>
			<br/>
			<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>DISTANCE: %4m</t>
		", _commandType, _text2, _text3, (_text2 * 60)];
	};
