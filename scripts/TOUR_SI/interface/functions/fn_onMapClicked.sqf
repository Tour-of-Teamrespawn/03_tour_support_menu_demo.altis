private 
[
	"_display", "_map", "_textHelp", "_buttonExecute", "_comboRoundsType", "_comboSliderAmount", "_comboSliderRadius", "_comboSliderInterval", "_marker", "_x", "_y", "_pos", "_mkrnumber", "_marker2"
];
...
}else
{
	...
	if (toLower _supportType == "airstrike") then
	{
		if (str (getMarkerPos "TOUR_SI_markerAS") == "[0,0,0]") then
		{
			_marker2 = createMarkerLocal ["TOUR_SI_markerAS", _pos];
		}else
		{
			_marker2 = "TOUR_SI_markerAS";
			_marker2 setMarkerPosLocal _pos;
		};
		_marker2 setMarkerShapeLocal "RECTANGLE";
		_marker2 setMarkerSizeLocal [20,20];
...
{	
	...
	"
		<img size='3' image='\a3\Ui_f\data\GUI\Cfg\CommunicationMenu\artillery_ca.paa'/> <t size='1.25' font='puristaMedium'>AIRSTRIKE SUPPORT</t>
		<br/>
...
{	
	...
	"
		...
		<br/>
		<t size='0.85' font='TahomaB' color='#D0D0D0' align='left' valign='top'>Use the options below and select a point on the map to customize your air strike, and press EXECUTE FIRE MISSION.</t>
	";
};
